...
static NSString * formatTimeInterval(CGFloat seconds, BOOL isLeft)
{
    ...
    if (h != 0) [format appendFormat:@"%d:%0.2d", (int)h, (int)m];
    else        [format appendFormat:@"%d", (int)m];
    [format appendFormat:@":%0.2d", (int)s];
...
- (void) handlePan: (UIPanGestureRecognizer *) sender
{
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        const CGFloat sp = MAX(0.1, log10(fabsf((float)vt.x)) - 1.0);
        const CGFloat sc = fabsf((float)pt.x) * 0.33 * sp;
        if (sc > 10) {
...
-(void) play
{
    ...
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, 0.250 * NSEC_PER_SEC);
    ...
    @autoreleasepool {
        ...
        while (numFrames > 0) {
            ...
            if (_currentAudioFrame) {
                ...
                memmove(outData, bytes, bytesToCopy);
...
- (void) asyncDecodeFrames
{
    ...
    const CGFloat duration = _decoder.isNetwork ? 1.0f : 1.0f;
